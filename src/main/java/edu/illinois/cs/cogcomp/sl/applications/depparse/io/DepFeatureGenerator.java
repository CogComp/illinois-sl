...
public class DepFeatureGenerator extends AbstractFeatureGenerator {
	...
	public IFeatureVector getFeatureVector(IInstance x, IStructure y) {
		...
		extractFeatures(sent, tree, featureMap);
	...
	private void extractFeatures(DepInst sent, DepStruct tree,
			List<String> featureMap) {
		for (int i = 0; i < sent.size(); i++) {
			int head = tree.heads[i+1];
			System.out.println("handling "+head+" "+(i+1));
			addUnigramFeats(head, i+1, sent, featureMap);
			addBigramFeats(head, i+1, sent, featureMap);
			addSurroundingPOS(head, i+1, sent, featureMap);
		}
	}
	...
	private void addSurroundingPOS(int head, int i, DepInst sent,
		...
		String nexthead=(head+1 < sent.pos.length) ? sent.pos[head+1] : "NULL";
		String prevmod=(i-1 > 0) ? sent.pos[i-1] : "NULL";
		String prevhead=(head-1 > 0) ? sent.pos[head-1] : "NULL";
		flist.add("headpos_"+sent.pos[head]+"_nexthead_"+nexthead+"_modpos_"+sent.pos[i]+"_nextmod_"+nextmod);
		flist.add("headpos_"+sent.pos[head]+"_prevhead_"+prevhead+"_modpos_"+sent.pos[i]+"_nextmod_"+nextmod);
		flist.add("headpos_"+sent.pos[head]+"_nexthead_"+nexthead+"_modpos_"+sent.pos[i]+"_prevmod_"+prevmod);
		flist.add("headpos_"+sent.pos[head]+"_prevhead_"+prevhead+"_modpos_"+sent.pos[i]+"_prevmod_"+prevmod);
	...
	private void addBigramFeats(int head, int i, DepInst sent,
		...
		featureMap.add("headpos_" + sent.pos[head] + "_modifierboth_" + sent.lemmas[i]
				+ "_" + sent.pos[i]);
