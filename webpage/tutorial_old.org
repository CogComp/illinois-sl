...
It houses learning algorithms like averaged Structured Perceptron and Structured SVM with L-2 Loss, and provides a minimal interface for your structured learning needs.
The training algorithm employed for training SSVM is dual coordinate descent(DCD), which has been proven to have very good convergence properties. Illinois-SL comes with an efficient implementation of DCD with support for multi-threading.
...
#+END_SRC
* TODO Example 2 - Multiclass Classification
\begin{align*}
\w = \begin{bmatrix}
\w_1 & \w_2 & \cdots & \w_M
\end{bmatrix} \\
...
\end{align*}
* Final Remarks
You can find the source code for the POS tagging example at <PUT LINK HERE>.
You can download a small dataset for training the model at <PUT LINK HERE>.
Here is a simple script to run the POS tagging example. It will train your model, and test it on a test dataset.

* Contact
You can contact the authors at [[mailto:kchang10@illinois.edu][kchang10@illinois.edu]] or [[mailto:upadhya3@illinois.edu][upadhya3@illinois.edu]] in case you have a bug to report or have any comments/feedback/questions. If you use this package we will be happy if you cite us,
